...
# load and prepare data
	...
	mypca <- PCA(scale(DM.racc, scale=FALSE))

	# set colors
	mycol <- rep(1,times=nrow(DM.racc))
	mycol[which(DM.desc$anno == 2011)] = "red"
	mycol[which(DM.desc$anno == 2012)] = "black"
	...
	mypch <- rep(1,times=nrow(DM.racc))
	mypch[which(DM.desc$tesi == 1)] = 1
	mypch[which(DM.desc$tesi == 3)] = 19
	mypch[which(DM.desc$tesi == 2)] = 4


	scoreplot(mypca, col=mycol, pch=mypch)


	biplot(mypca,  score.col=mycol, show.names=c("loadings"))

	pairs(scores(mypca)[,1:4], col = mycol, pch = mypch)
